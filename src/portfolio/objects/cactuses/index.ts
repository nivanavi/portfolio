...
const getModelByType = (type: cactusTypes) => {
  switch (type) {
    case "bigStone":
...
export const cactusObject = (props: objectProps, type: cactusTypes) => {
  ...
  gltfLoader.load(
    ...
    model => {
      ...
      cactusModel.children.forEach(child => child.castShadow = true);
  ...
  cactusBody.addEventListener("collide", (ev: any) => {
